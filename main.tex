...
\vspace{0.3cm}
\begin{tabular}{ll}
Luiz Victor & \pendente{matrícula} \\
Marcus & \pendente{matrícula} \\
\pendente{nome} & \pendente{matrícula} \\
\pendente{nome} & \pendente{matrícula} \\
\end{tabular}
...
\midrule
Processador & Intel Xeon @ 2.10\,GHz (1 núcleo disponível) \\
Cache & L1d 48\,KiB; L2 2\,MiB; L3 260\,MiB \\
Memória (RAM) & 3\,GB \\
...
\noindent
\pendente{se o grupo reexecutar o experimento em outra máquina --- o que é
recomendável --- substituir os dados da Tabela~\ref{tab:ambiente} pelos da
máquina utilizada. As tabelas e figuras numéricas são regeradas
automaticamente pelo script \texttt{codigo/gerar\_graficos.py}}
...
\begin{verbatim}
main.tex                                  este relatorio
codigo/dijkstra_experimento.cpp           implementacao das 3 variantes
codigo/gerar_graficos.py                  agregacao, figuras e tabelas
resultados/resultados_brutos.csv          todas as medicoes individuais
resultados/estatisticas_estruturais.csv   Dmax e varreduras de bucket
resultados/tabelas.tex                    tabelas geradas (incluido no .tex)
resultados/log_execucao.txt               log, com o resultado da validacao
figuras/*.pdf                             graficos gerados
\end{verbatim}

Para reproduzir o experimento inteiro:

\begin{verbatim}
cd codigo
g++ -O2 -std=c++17 -o dijkstra dijkstra_experimento.cpp
./dijkstra > ../resultados/resultados_brutos.csv \
            2> ../resultados/log_execucao.txt
./dijkstra stats > ../resultados/estatisticas_estruturais.csv
python3 gerar_graficos.py          # requer matplotlib
cd .. && pdflatex main.tex && pdflatex main.tex
\end{verbatim}

\noindent
Como as tabelas e figuras do relatório são geradas a partir dos CSV, basta
reexecutar esses comandos em outra máquina para que todos os números do
documento sejam atualizados. Tempo total de execução na máquina da
Tabela~\ref{tab:ambiente}: cerca de $1$~minuto e $30$~segundos, dominado pela
variante Normal em $n = 64000$.

\noindent
\textbf{Repositório do grupo:} \pendente{link do repositório, se houver}
...
\lstinputlisting[caption={\texttt{codigo/dijkstra\_experimento.cpp}}]{codigo/dijkstra_experimento.cpp}
